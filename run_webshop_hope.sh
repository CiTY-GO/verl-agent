...
ENV_WEBSHOP=$BASE/../miniconda3/envs/webshop-cty
...
if [ ! -f "$DATA_DIR/train.parquet" ]; then
    ...
    mkdir -p $DATA_DIR
    cd $VERL_AGENT
    ${ENV_WEBSHOP}/bin/python3 -m examples.data_preprocess.prepare \
        --mode 'text' \
        --train_data_size 16 \
        --val_data_size 256
else
...
# ============================================================
# Step 2: 设置环境变量，使用 webshop-cty conda 环境
# ============================================================
cd $VERL_AGENT
# web_agent_site 加入 PYTHONPATH，让镜像 Python 也能 import
export PYTHONPATH=$VERL_AGENT:$VERL_AGENT/agent_system/environments/env_package/webshop/webshop:$PYTHONPATH
export PYTHONNOUSERSITE=1  # 避免 ~/.local 包污染 conda 环境
...
# ============================================================
# Step 3: 生成时间戳 checkpoint 目录
# ============================================================
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
CKPT_DIR=$BASE/experiments/checkpoints/webshop/gigpo_webshop_1.5b_${TIMESTAMP}
mkdir -p $CKPT_DIR
...
${ENV_WEBSHOP}/bin/python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='gigpo_webshop_1.5b' \
