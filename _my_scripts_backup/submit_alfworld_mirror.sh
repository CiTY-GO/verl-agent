...
# 训练命令：直接调用 run_alfworld_hope.sh（使用镜像自带环境）
# ray_init.num_cpus=80：给 Ray 足够的 CPU 资源（AlfworldWorker 每个占 0.2 CPU，
# train_batch_size=16 * group_size=8 = 128个并行环境，需要 128*0.2=25.6 CPU，
# 加上 Actor/Rollout worker 等，80 CPU 足够）
